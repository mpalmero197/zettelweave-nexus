-- Fix 1: Modify search_users to hide emails for non-friends
...
BEGIN
  ...
  SELECT 
    au.id as user_id,
    -- Only show email if user is a friend, otherwise hide it
    CASE 
      WHEN public.are_friends(auth.uid(), au.id) THEN au.email::text
      ELSE NULL
    END as email,
...
-- Fix 2: Modify get_all_visible_users to hide emails for non-friends
...
-- Fix 3: Remove overly permissive RLS policies on cookie_consent_analytics
DROP POLICY IF EXISTS "Anyone can insert cookie consent" ON cookie_consent_analytics;

-- Require session_id to be non-empty for inserts
CREATE POLICY "Anyone can insert cookie consent with valid session"
ON cookie_consent_analytics FOR INSERT
WITH CHECK (session_id IS NOT NULL AND session_id != '');

-- Fix 4: Remove duplicate permissive INSERT policy on security_audit_log
